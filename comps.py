"""
Hardware components of MIPS CPU core.
Though originally implemented as asynchronous components for interactive purposes,
I figured I'll just make it synchronous since I won't be making any GUI or real
fun CPU simulation.

The component parent class describes most of the code format, all components
have inputs in the form of a dictionary to callbacks that receive the 
data. Those functions are set in the bind function. There is an output
dictionary for outputs generated by the component.
There is also a tick function for calculating state/outputs based on inputs.

The ClockedComponent class is deprecated and was there for the original async design.

The Inspector component could be considered for refactoring into the CPU itself, but the
CPU object is large enough, defining all the components and bindings.

Though I do have a primitive assembler with some traces of decent design,
a proper MIPS CPU simulator should probably involve:

- a MIPS to x86(_64) transpiler (and assembler)
- a small kernel for interactive execution
- a GUI component for visualization
...
class Control(Component):
    ...
    def tick(self):
        ...
        if op == '000000':  # R-format
            ...
            raise Exception(f'unknown opcode, {op}, in instruction')
...
# TODO: make it preserve padding +2
...
class JumpConcatenator(Component):
    def __init__(self):
        super().__init__()
        self.ins['in_1'] = Bint(0)
        self.ins['in_2'] = Bint(0)
...
class Inspector(ClockedComponent):
    ...
    def lastcycle(self):
        ...
        for i in range(8):
            for j in range(4):
                ...
                regdump += f'${str(4*i+j).zfill(2)}={r.hex()}({r.dec(True).rjust(11)})    '
            regdump += '\n'
        print(regdump, end='')
    ...
    def eachcycle(self):
        print(f'{42*"="}\nCycle {self.cyclec}')
        #funct, immediate, jump address, write regsiter
        pc = self.pc.reg.val
        print(f'PC={pc.hex()} {pc.dec()}')
        instr = self.instrmem.outs['read']
        print(f'instruction={instr.hex()} {instr.dec()} >>> {instr.bin()}')
        op = self.instrmem.outs['read'][0:6]
        print(f'opcode={op.bin()} {op.dec()}')
        funct = self.instrmem.outs['read'][26:32]
        print(f'funct={funct.bin()} {funct.dec()}')
        ...
        print(f'immediate={imm.hex()} {imm.dec()}')
        ...
        jumpaddr = self.jumpmux.ins['in_2']()
        print(f'Jump_address={jumpaddr.hex()} {jumpaddr.dec()} >>> {jumpaddr.bin()}')
        ###############################
        # XXX
        catin1 = self.jumpcat.ins['in_1']()
        print(f'JumpCatIn1={catin1.hex()} {catin1.bin()}')
        catin2 = self.jumpcat.ins['in_2']()
        print(f'JumpCatIn2={catin2.hex()} {catin2.bin()}')
        catout = self.jumpcat.outs['out']
        print(f'JumpCatOut={catout.hex()} {catout.bin()}')
        jsin = self.jumpshift.ins['in']()
        print(f'JumpShiftIn={jsin.bin()}')
        jsout = self.jumpshift.outs['out']
        print(f'JumpShiftOut={jsout.bin()}')
        ###############################
        writereg = self.writeregmux.outs['out']
        print(f'Write_register={writereg.bin(5)} {writereg.dec()}')
